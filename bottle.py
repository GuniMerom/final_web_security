...
def redirect(url, code=None, add_cookies=None, clear_cookies=None):
    ...
    if add_cookies:
        for cookie_ in add_cookies:
        	res.set_cookie(cookie_[0], cookie_[1]) # patch that saves redirect's cookies
    if clear_cookies:
        for cookie_ in clear_cookies:
        	res.delete_cookie(cookie_) # patch that saves redirect's cookies
        print("not ready yet")	
    res.status = code
    res.body = ""
